...
export default function TimezoneChangeDialog() {
  ...
  const [currentTz, setCurrentTz] = useState(dayjs.tz.guess() || 'Europe/London');
  const { t } = useLocale();
  const { data: user, isLoading } = trpc.viewer.me.useQuery();
  const utils = trpc.useContext();
  const userTz = user?.timeZone;
  ...
  useEffect(() => {
    if (router.isReady) {
      const urlTimezone = router.query.tz;
      if (urlTimezone && typeof urlTimezone === 'string') {
        setCurrentTz(urlTimezone);
      }
    }
  }, [router.isReady, router.query.tz]);
  ...
  const THREE_MONTHS = ONE_DAY * 90; // 90 days in seconds
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        ...
        description={t('update_timezone_description', { formattedCurrentTz })}
        type='creation'
        onInteractOutside={() => onCancel(ONE_DAY, false)}>
        <div className='mb-8' />
        <DialogFooter showDivider>
          <DialogClose onClick={() => onCancel(THREE_MONTHS, true)} color='secondary'>
            {t('dont_update')}
